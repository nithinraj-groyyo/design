...
const CatalougePageDetails = () => {
  ...
  const [fetchCatalogues, { data, isLoading: isProductLoading }] =
    useLazyFetchCatalogueListQuery();

  const { catalogueId } = useParams<{ catalogueId: string }>();

  const {
    data: catalogueDetials,
    isLoading,
    isError,
    error,
  } = useFetchCatalogueByIdQuery({
  ...
  const getCurrentPage = () => {
    if (flipBookRef.current) {
      const currentPageIndex = flipBookRef.current
        .pageFlip()
        .getCurrentPageIndex();
            ...
            <div className="flex flex-col items-center mt-4">
              <div className="flex justify-center gap-6 items-center">
                <motion.button
                  ...
                  className="p-2 rounded-full bg-white bg-opacity-45 text-black font-semibold shadow-md flex items-center justify-center"
                  whileHover={{ scale: 1.1 }}
                  transition={{ type: "spring", stiffness: 300 }}
                >
                ...
                <motion.button
                  ...
                  className="p-2 rounded-full bg-white bg-opacity-45 text-black font-semibold shadow-md flex items-center justify-center border "
                  whileHover={{ scale: 1.1 }}
                  transition={{ type: "spring", stiffness: 300 }}
                >
